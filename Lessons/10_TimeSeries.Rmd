...
> aggregation: Can help fill gaps by changing time frame (daily -> monthly). 

> subsampling: Picking out individual values (also can convert daily-> monthly), but takes individual data instead of combining.

> interpolation: connecting a gap in data
...
ggplot(subset(NeuseCond_monthly,!is.na(Month)),
  ...
  geom_boxplot()#+
  #geom_hline(yintercept=mean(NeuseCond_monthly$Conductivity),color=red)
...
> Most significant increases in jan-march, also increase in discharge. This means a much higher increase in salinizing agents. Top conductivity is staying the same, but base conductivity is increasing. 
