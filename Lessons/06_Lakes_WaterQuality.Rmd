...
options(scipen = 100)
#tells R to report this many digits before simplifying in outputs
...
#if the two data frames have different column names with same info, in left_join... by=c("column1" = "column2")
...
LAGOStrophic <- LAGOSnutrient %>%
  ...
  select(lagoslakeid, sampledate, chla, tp, secchi,tn, 
         gnis_name, lake_area_ha, state, state_name) %>%
  mutate(sampleyear = year(sampledate), 
         samplemonth = month(sampledate)) %>%
  drop_na(chla, tp, tn,secchi, state)
...
#Preserve="single" allows blank space for missing classes instead of extending existing classes to fill space
```
