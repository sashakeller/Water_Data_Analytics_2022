...
summary(lm(data=EnoDischarge, Baseflow_lfstat ~ bt))
...
ggplot(EnoDischarge_18_20, aes(x = Date, y = Discharge)) +
  geom_line(color = "gray") +
  geom_line(aes(y = Baseflow_lfstat), color = "darkred", size =1.3) +
  geom_line(aes(y = bt),size=1) +
  scale_y_log10() +
  labs(x = "Year", y = "Discharge (cfs)")
...
EnoDischarge_summary <- EnoDischarge %>%
  group_by(WaterYear) %>%
  summarise(Discharge.acft.yr = sum(Discharge)*723.968, # what is this factor? cubic ft per second -> acre feet per year
...
ggplot(EnoDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line() +
  geom_line(aes(y = Baseflow.acft.yr), lty = 2) #lty = line type. this makes it dashed. good details.
...
DodgeDischarge<- DodgeDischarge %>%
  mutate(Baseflow=baseflow(Discharge),
         Stormflow=Discharge-Baseflow,
         Year=year(Date),
         WaterYear=water_year(Date))
DodgeDischarge$WaterYear<-as.numeric(as.character(WaterYear))


ggplot(DodgeDischarge, aes(x=Date,y=Discharge))+
  geom_line()+
  geom_line(y=DodgeDischarge$Baseflow, color="blue")
  #scale_y_log10()
```
