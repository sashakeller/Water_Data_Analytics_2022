...
#can use drop_na() to remove the entries under a given column (for example, missing state names) to visualize data more appropiately without removing other important entries in a given row (for example, missing chlorophyll but has total P)
```
...
LAGOSMN <- LAGOSjoined %>%
  ...
  LAGOSMN$sampledate<-as.Date(LAGOSMN$sampledate, form="%y/%m/%d")%>%
  mutate(Month = month(sampledate),
         Year=year(sampledate))%>%
  filter(state_name="Minnesota")%>%
  filter(LAGOSMN$Year>="1990")%>%
  filter(LAGOSMN$Month %in% c(6:9))
