...
> Seasonality is maybe a factor, no obvious data gaps, lots of variability. Seems unlikely that linear model would be appropriate. 
...
> Less variable, more aggregated data.
...
* **Piecewise constant**: aka nearest neighbor. replicates nearest points for missing data?
* **Linear**: draws straight line between data points
* **Spline**: 
...
```{r}
NeuseCond_ts<-ts(NeuseCond_monthly[[3]],frequency=12)
