...
```{r}
LakeMe<-filter(LAGOScombined, state =="ME")

secchi.me.summary <- LakeMe %>%
  group_by(lagoslakeid) %>%
  summarise(secchi.mean = mean(secchi),
            area = mean(lake_area_ha),
            depth = mean(maxdepth),
...
```{r}
secchi.me.spatial <- st_as_sf(secchi.me.summary, coords = c("long", "lat"), crs = 4326)

states.me.subset <- filter(states, ID %in% 
                          "maine")

MEArea<-ggplot() +
  geom_sf(data = states.me.subset, fill = "white") +
  geom_sf(data = secchi.me.spatial, aes(color = secchi.mean, size = area), 
          alpha = 0.5) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
...
MEDepth<-ggplot() +
  geom_sf(data = states.me.subset, fill = "white") +
  geom_sf(data = secchi.me.spatial, aes(color = secchi.mean, size = depth), 
          alpha = 0.3) +
  scale_color_viridis_c(trans = "log10", end = 0.8, direction = -1) +
  labs(color = "Average Secchi Depth (m)") +
  theme(legend.position = "top")
