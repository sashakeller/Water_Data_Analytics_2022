...
ggplot(MysterySite.Annual.Full, aes(x=WaterYear, y=RecurrenceInterval))+
  geom_point()+
  geom_point(data=MysterySite.Annual.30yr, color="orange",aes(x=WaterYear, y=RecurrenceInterval))+
  scale_y_continuous("Recurrence Interval", limits=c(0, 58))
...
> The recurrence intervals coinicinded surprisingly well. The two highest points reached fairly distinct levels of disparity, but the majority of points were the same across the overlapping 30 year period. This indicates a significant degree of stationarity for this river. The discrepency could indicate some changes in the latter 30 year portion (potentially climate change), but it is relatively quite stable.
